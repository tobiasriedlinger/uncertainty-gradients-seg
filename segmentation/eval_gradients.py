...
parser.add_argument('--sliding_overlap', type=float, default=0)
...
parser.add_argument('--crop_size', type=int, default=2048)  # 1024)
        ...
        for row in range(tile_rows):
            for col in range(tile_cols):
                ...
                if y1 < 0:
                    y1 = 0
                if crop_ctr == 0:
                    sliding_window_cropping = data[img_ctr][:, y1:y2, x1:x2].unsqueeze(
                        0)
...
def reverse_mapping(i, ctr, input_img, mapping, que, flip, origw, origh):
    ...
    full_probs = np.zeros((args.dataset_cls.num_classes+20, h, w))
    count_predictions = np.zeros((args.dataset_cls.num_classes+20, h, w))
...
def pooled_eval(model, image, scale):
    ...
    with torch.no_grad():
        ...
        if scale > 1.0:
            ...
            y = [torch.nn.Upsample(
                scale_factor=2, mode='bilinear')(y_) for y_ in y]
        else:
            pass
    ...
    for scale in scales:
        ...
        with torch.no_grad():
            ...
            if hi >= args.crop_size:
                ...
                output_scattered = model(input_crops, memory=True)
...
class RunEval():
    ...
    def inf(self, imgs, img_names, gt, inference, net, scales, pbar, base_img):
        ...
        compose_img_name = '{}/{}_compose.png'.format(
            self.compose_path, self.img_name)
        ...
        if self.inference_mode == 'pooling':
            ...
            unc_heatmaps = prediction_pre_argmax[-20:, ...]
            prediction_pre_argmax = prediction_pre_argmax[:-20, ...]
            prediction = np.argmax(prediction_pre_argmax, axis=0)

        # save softmax outputs
        prediction_pre_argmax = self.softmax(prediction_pre_argmax)
        with open(pred_img_name.replace(".png", ".npy"), 'wb') as filename:
            np.save(filename, np.concatenate(
                [prediction_pre_argmax, unc_heatmaps], axis=0).astype("float32"))
